...
export default function GalleryBlock({ isHeader = false, userId }: { isHeader?: boolean, userId: string | undefined }) {
    ...
    const [images, setImages] = useState<{ id: string; imageUrl: string; userId: string; }[]>([]);
    const [role, setRole] = useState<string | undefined>(undefined);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [imageUrl, setImageUrl] = useState<string | null>(null);
    const imageSave = trpc.project.createHomePageGallery.useMutation()
    const { mutateAsync: deleteImage } = trpc.project.deleteHomePageGallery.useMutation();
    const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
    const [deleteImageId, setDeleteImageId] = useState<string | null>(null);
    const [deleteImageUrl, setDeleteImageUrl] = useState<string | null>(null);
    const [submitButtonDisabled, setSubmitButtonDisabled] = useState(true);
    const {data: projectWithGalleryFromApi, isLoading: isProjectWithGalleryLoading} = trpc.project.getHomePageGallery.useQuery();
...
useEffect(() => {
    if (projectWithGalleryFromApi) {
        setImages(projectWithGalleryFromApi);
    }
}, [projectWithGalleryFromApi]);
